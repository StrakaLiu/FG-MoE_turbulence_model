...
void FG_MoE<BasicTurbulenceModel>::correctNonlinearStress(const volTensorField& gradU)
{
    ...
    volTensorField SS_ = S & S;
    volTensorField SW_ = S & W;
    volTensorField WW_ = W & W;
    volScalarField IIIs = tr(SS_ & S);
    volScalarField IV = tr(SW_ & W);
    volScalarField V = tr(SS_ & WW_);
...
void FG_MoE<BasicTurbulenceModel>::correct()
{
    ...
    const volScalarField nut0(k_/omega_);
